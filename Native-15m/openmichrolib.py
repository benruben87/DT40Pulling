...
class Akroma:
    ...
    def _initGrosbergBondForce(self):
        ...
        if "GrosbergBondForce" not in list(self.forceDict.keys()):
            ...
            #force = (" (10 * GROSk * (r-GROSrmax)+(- 0.5 * GROSk * GROSr0 * GROSr0 * log(1-(GROSrmax/GROSr0)*(GROSrmax/GROSr0)))) * step(r-GROSrmax) +  (- 0.5 * GROSk * GROSr0 * GROSr0 * log(1-(r/GROSr0)*(r/GROSr0)))*step(GROSrmax-r) + (4 * GROSe * ((GROSs/r)^12 - (GROSs/r)^6) + GROSe) * step(GROScut - r)*step(r-GROSrmin) + (10*GROSk*(GROSrmin-r) + 4 * GROSe * ((GROSs/GROSrmin)^12 - (GROSs/GROSrmin)^6) + GROSe) * step(GROSrmin - r)") #Uses 10*GROSk as the constant extension force.  Softer than a continuous extension to prevent blow-up.
            
            force = ("- 0.5 * GROSk * GROSr0 * GROSr0 * log(1-(min(r, GROSrmax)/GROSr0)*(min(r, GROSrmax)/GROSr0)) + (4 * GROSe * ((GROSs/max(r, GROSrmin))^12 - (GROSs/max(r, GROSrmin))^6) + GROSe) * step(GROScut - r) + (GROSk*GROSrmax*(1-(GROSrmax/GROSr0)^2)^-1 * (r-GROSrmax))*step(r-GROSrmax) + (4 * GROSe/GROSrmin * (12*(GROSs/GROSrmin)^12 - 6*(GROSs/GROSrmin)^6)*(GROSrmin-r)*step(GROSrmin-r))")
            
            bondforceGr = self.mm.CustomBondForce(force)
            bondforceGr.addGlobalParameter("GROSrmin", 0.8)
    ...
    def doBlock(self, steps=None, increment=True, num=None, reinitialize=True, maxIter=0, output=True, checkFunctions=[]):
        ...
        for attempt in range(15):
            ...
            if ((np.isnan(newcoords).any()) or (eK > self.eKcritical) or
                ...
                print('FAILED Block = ' + str(self.step))
                self.printForces()
                failfname = 'failed_'+str(self.step)+'.pdb'
                self.save(mode = 'pdb', pdbGroups=self.chains, filename = failfname) 
