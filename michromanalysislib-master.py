...
def find_opt_bead_size(trajFile, BondLens = [], eqFrames = 0, N_init = 600, volFraction = .95, maxIter = 30):
    ...
    if ctr == maxIter:
        print('Maximum Iterations Reached!  Returning Current Value.')
...
#LinearTime: Calculates MSD at every delta_tau up to this time, then at logarithmically spaced times after this.
#TimesPerDecade: For logarithmically spaced time regime (t>LinearTime), the number of time points per decade along time axis.
def calc_bead_MSDs_LogTime(trajFile, 
                           max_tau = 1000, 
                           dt_perFrame = .1, 
                           chronum=7, 
                           delta_tau = .1,
    ...
    MSD_Matrix = [] #MSD time course for each bead
    timevec = [] #Corresponding times for each column of the MSD_Matrix
    ...
    #Makes vector of frame jumps corresponding to the sampled time differences.
    max_jump = int(max_tau/dt_perFrame)
    MaxLinearJump = int(LinearTime/dt_perFrame)
    jumpjump = int(delta_tau/dt_perFrame)
    
    Linear_Frame_Jumps = range(1, MaxLinearJump+1, jumpjump)
    
    numLogJumps = int(TimesPerDecade*np.log10(max_jump-MaxLinearJump))
    Logarithmic_Frame_Jumps = np.logspace(np.log10(MaxLinearJump+jumpjump), np.log10(max_jump), num=numLogJumps).tolist()
    Logarithmic_Frame_Jumps = [int(l) for l in Logarithmic_Frame_Jumps]
    
    allFrameJumps = list(Linear_Frame_Jumps)+Logarithmic_Frame_Jumps
    
    print('####################################################################')
    print('MSD Calculation for Chromosome: Chro {:} with {:} beads'.format(chronum, (len(chro[firstkey]))))
    print('Using {:} Linear Bins up to time {:} and {:} logarithmic bins up to time {:}'.format(MaxLinearJump//jumpjump, LinearTime, numLogJumps, max_tau))

    for i_del, delta_frame in enumerate(allFrameJumps):
        ...
        if (i_del)%(len(allFrameJumps)//10)==0:
...
#dt_perFrame = time between capturedconformations. For a time step of .01 and block size of 10, dt_perFrame=.1
#COM: If true, subtracts off the center of mass motion of the whole chromosome.
#LinearTime: Calculates MSD at every frame up to this time, then at logarithmically spaced times after this.
#TimesPerDecade: For logarithmically spaced time regime (t>LinearTime), the number of times per decade along time axis.
